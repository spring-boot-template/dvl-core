...
public class ImageUtils {
	...
	public static int[] hsbPallete(int w, int h) {
		...
		for (int i = 0; i < pixels.length; i++) {
			...
			float hue = MxQuery.$(x, 0, w).map(0, 1).f();
			float sb = MxQuery.$(y, 0, h).map(0, 1).f();
	...
	public static BufferedImage randomPattern(Dimension dimension) {
		...
		for (int i = 0; i < pixels.length; i++) {
			...
			float hue = MxQuery.$(hAngle, -1, 1)
					.map(hRange1 * fixer, hRange2 * fixer).f();
			float saturation = MxQuery.$(sAngle, -1, 1)
					.map(sRange1 * fixer, sRange2 * fixer).f();
			float brightness = MxQuery.$(bAngle, -1, 1)
	...
	public static BufferedImage randomPattern1(Dimension dimension) {
		...
		for (int i = 0; i < pixels.length; i++) {
			...
			float hue = MxQuery.$(sin, -1, 1).map(0, 1).f();
			float saturation = MxQuery.$(s, -1, 1).map(0, 1).f();
			float brightness = MxQuery.$(b, -1, 1).map(0, 1).f();
